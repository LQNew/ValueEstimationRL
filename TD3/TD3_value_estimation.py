"""Example for computing value estimation bias in MuJoCo"""
import copy
import numpy as np
import torch
...
# Returns Q-values for given state/action pairs
...
class TD3_ValueEstimate(object):
	def __init__(
		self,
		state_dim,
		action_dim,
	...
	def select_action(self, state, return_critic_value=False):
		state = torch.as_tensor(state.reshape(1, -1), device=device, dtype=torch.float32)
		action = self.actor(state)
		if return_critic_value:
			Q1, Q2 = self.critic(state, action)
			return action.cpu().data.numpy().flatten(), ((Q1 + Q2) / 2).mean().cpu().data.numpy()
		else:
	...
	def predict_V(self, state):
		optimal_action = self.actor(state)
		Q1, Q2 = self.critic(state, optimal_action)
		return ((Q1 + Q2) / 2).mean().cpu().data.numpy()
